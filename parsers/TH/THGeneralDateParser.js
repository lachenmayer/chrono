...
(function() {
  ...
  function THGeneralDateParser(text, ref, opt) {
    ...
    parser.extract = function(full_text, index) {
      ...
      var date = null;
      if (text == 'วันนี้') date = moment(ref).clone();
      else if (text == 'พรุ่งนี้') date = moment(ref).clone().add('d', 1);
      else if (text == 'เมื่อวาน') date = moment(ref).clone().add('d', -1);
      else if (text == 'เมื่อคืน') date = moment(ref).clone().add('d', -1);
      else {
  ...
  exports.THGeneralDateParser = THGeneralDateParser;
