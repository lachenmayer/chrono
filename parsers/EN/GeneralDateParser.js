(function() {
  ...
  function GeneralDateParser(text, ref, opt) {
    ...
    parser.extract = function(full_text, index) {
      ...
      if (lowercase_text == 'today') date = moment(ref).clone();
      else if (lowercase_text == 'tomorrow') date = moment(ref).clone().add('d', 1);
      else if (lowercase_text == 'yesterday') date = moment(ref).clone().add('d', -1);
      else if (lowercase_text.match('last')) date = moment(ref).clone().add('d', -1);
      else if (lowercase_text.match('ago')) {
        var days_ago = matchedTokens[2];
        days_ago = parseInt(days_ago, 10);
  ...
  exports.GeneralDateParser = GeneralDateParser;
})();
