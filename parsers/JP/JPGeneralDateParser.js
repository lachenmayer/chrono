(function() {
  ...
  function JPGeneralDateParser(text, ref, opt) {
    ...
    parser.extract = function(full_text, index) {
      ...
      var date = null;
      if (text == '今日') date = moment(ref).clone();
      else if (text == '明日') date = moment(ref).clone().add('d', 1);
      else if (text == '昨日') date = moment(ref).clone().add('d', -1);
      else {
  ...
  exports.JPGeneralDateParser = JPGeneralDateParser;
