(function() {
  ...
  function JPStandardDateParser(text, ref, opt) {
    ...
    opt = opt || {};
    ref = ref || new Date();
    var parser = require('./JPGeneralDateParser.js').JPGeneralDateParser(text, ref, opt);
    ...
    parser.extract = function(full_text, index) {
      ...
      if (years && !isNaN(years)) {
        ...
        if (date.format('YYYY-M-D') != dateText) return null;
      } else {
  ...
  exports.JPStandardDateParser = JPStandardDateParser;
